 ...
 * @param {number} [timeout_ms := 1000] - Timeout (in milliseconds). Basically, there is no need to set this.
...
ExecuteVsCodeCommand(commandName, timeout_ms := 1000) {
  ...
  while (true) {
    ...
    if (timeout_ms < elapsedTime) {
      throw Exception("Timeout. " commandName " was executed.")
    }
    Sleep, 1
  }
